...
\begin{tabularx}{\linewidth}[ht]{@{}LR@{}}
	...
	\cmd{ls -a}									& list hidden contents of the current directory \hlx
	\cmd{cd}									& change the working directory to home \\
	\cmd{cd \vb{DIR}}							& change the working directory to \vb{DIR} \hlx
...
\begin{tabularx}{\linewidth}[ht]{@{}LR@{}}
	...
	\code{exit}					& log out of the current session \hlx
	\code{clear}				& clear the terminal screen \\
	\mybottomrule
\end{tabularx}
...
\begin{tabularx}{\linewidth}[ht]{@{}LR@{}}
	...
	\code{\ctrlp \shiftp I}		& open a new window \\
	\mybottomrule
\end{tabularx}
\spacebtwtables
...
\begin{tabularx}{\linewidth}[ht]{@{}LR@{}}
	...
	\cmd{locate \vb{FILE\_NAME}}			& find files whose name contain \vb{FILE\_NAME} \\
	\mybottomrule
\end{tabularx}
...
\begin{tabularx}{\linewidth}[ht]{@{}LR@{}}
	...
	Appending \code{{-}{-}help} after a command will print command usage help. \\
	\mybottomrule
\end{tabularx}
...
\begin{tabularx}{\linewidth}[ht]{@{}LR@{}}
	...
	\cmd{catkin clean \vb{PACKAGE}}		& clean \vb{PACKAGE} \\
	...
	\cmd{wstool merge}					& merge workspace with another config set \\
...
\begin{tabularx}{\linewidth}[ht]{@{}L@{}}
	...
	{ \begin{tabularx}{\linewidth}[ht]{@{}L@{}}
		...
		\white{\cmd{}}run node \vb{EXECUTABLE} from \vb{PACKAGE} (with \vb{PARAM} set to \vb{VALUE}) \\ % find a way to replace \white with \phantom
		\mybottomrule
	\end{tabularx} } \\
\end{tabularx}
\spacebtwtables
...
\begin{tabularx}{\linewidth}[ht]{@{}LB@{}}
	\multicolumn{2}{@{}l@{}}{\headbf{ROS Launch File Elements}} \\
	\mytoprule
	\tag{node}& launch a node \hlx
	\tag{param}& set a parameter on the parameter server \hlx
	\tag{remap}& declare a name mapping \hlx
	\tag{resparam}& set ROS parameters for the launch \hlx
	\tag{include}& include other roslaunch files \hlx
	\tag{env}& specify an environment variable for launched nodes \hlx
	\tag{arg}& declare an argument \hlx
	\tag{group}& group enclosed elements sharing a namespace or remap \\
	\mybottomrule
\end{tabularx}
...
% ---------- SMB Workspace ---------- %
\begin{tabularx}{\linewidth}[ht]{@{}LR@{}}
	\multicolumn{2}{@{}l@{}}{\headbf{SMB Workspace}} \\
	\mytoprule
	Key Packages						& \\
	\code{rovio}						& robust visual inertial odometry framework \\
	\code{maplab}						& visual-inertial mapping framework \\
	\code{voxblox}						& volumetric mapping library \\
	\code{apriltag}						& visual fiducial system \\
	\code{elevation\_mapping}			& produce elevation map around robot \\
	\code{traversability\_estimation}	& traversability mapping for rough terrain \\
	\code{icp\_mapper}					& iterative closest point based slam system \\
	\code{smb\_local\_planner}			& path planning system for SMB \hlx
	Configuration						& \hlx
	Launch Files						& \\
	\mybottomrule
\end{tabularx}
...
% ---------- ROS TF2 Structure ---------- %
\begin{tabularx}{\linewidth}[ht]{@{}LR@{}}
	\multicolumn{2}{@{}l@{}}{\headbf{ROS TF2 Structure}} \\
	\mytoprule
	stamp							& time stamp of transform \\
	frame\_id \& child\_frame\_id	& id of parent and child frame \hlx
	translation						& x, y, z \\
